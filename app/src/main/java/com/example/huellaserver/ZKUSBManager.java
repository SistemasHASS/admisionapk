...
public class ZKUSBManager {
    ...
    public UsbDevice getConnectedDevice() {
        ...
        for (UsbDevice device : usbManager.getDeviceList().values()) {
            // Puedes filtrar solo ZKTeco o devolver el primero encontrado
    ...
    public void initUSBPermission(int vid, int pid){
        ...
        if (!usbManager.hasPermission(usbDevice))
        {
            ...
            @SuppressLint("UnspecifiedImmutableFlag") PendingIntent pendingIntent = PendingIntent.getBroadcast(mContext, 0, intent, 0);
